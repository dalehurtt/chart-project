...
namespace Charts
{
    ...
    public class ExponentialAverageList : List<DailyAverageData>
    {
        ...
        public ExponentialAverageList(List<DailyStockData> values, string ticker, int numDays1, int numDays2)
        {
            ...
            NumDays2 = numDays2;
            int _i = -1, _j = -1, _k = -1;
            decimal mult1 = (2 / numDays1 + 1);
            decimal mult2 = (2 / numDays2 + 1);
            decimal yema1 = 0, yema2 = 0;
            decimal ema1 = 0, ema2 = 0;
            decimal sumema1 = 0, sumema2 = 0;
            try
            {
                for (var i = 0; i < values.Count; i++)
                {
                    ...
                    ema1 = (data.Close - yema1) * mult1 + yema1;
                    ema2 = (data.Close - yema2) * mult2 + yema2;
                    sumema1 += ema1;
                    sumema2 += ema2;
                    yema1 = ema1;
                    yema2 = ema2;
                    avgPrice1 = sumema1 / divDays1;
                    avgPrice2 = sumema2 / divDays2;
                    // Create data record
                    string signal = string.Empty;
                    if (data.Close >= avgPrice1 && data.Close >= avgPrice2)
                    {
